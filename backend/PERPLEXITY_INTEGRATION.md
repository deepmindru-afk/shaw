...
2. **LLM Decision**: The hybrid LLM adapter (OpenAI Responses / Anthropic / Gemini) decides to call `web_search`
...
User Speech
    ↓
LiveKit STT → Hybrid LLM Adapter (text-only)
    ↓
Tool Call Decision
    ↓
...
**Status**: ✅ Complete
**Agent Mode**: LiveKit hybrid text pipeline + Cartesia Sonic TTS

...
All Shaw sessions now run in a hybrid mode: microphone input is transcribed by LiveKit, routed through the Python agent, and the resulting text/tool output is synthesized with Cartesia or ElevenLabs before being published back to the room. The agent relies on the metadata provided at dispatch time to decide which model and voice to use.

## Session Metadata

`/v1/sessions/start` sends a JSON metadata payload to the LiveKit Agent Dispatch service. The worker expects the following keys:

| Key | Description |
|-----|-------------|
| `session_id` | Database identifier for logging and summaries |
| `model` | Fully-qualified model name such as `openai/gpt-5.1-nano`, `claude-sonnet-4-5`, or `google/gemini-2.5-pro` |
| `voice` | TTS descriptor (`cartesia/...` or `elevenlabs/...`) |
| `tool_calling_enabled` | Enables/disables function calling on the LLM |
| `web_search_enabled` | Controls whether the `web_search` tool is registered |
| `language` | Optional ISO language hint passed to TTS |

The worker no longer inspects any `realtime` flag—every session flows through the same text-only pipeline.

...
const DEFAULT_VOICE = 'cartesia/sonic-3:9626c31c-bec5-4cca-baa8-f8ba9e84c8bc';
...
const MODEL_ALLOWLIST = new Set([...OPENAI_MODELS, ...ANTHROPIC_MODELS, ...GOOGLE_MODELS]);
const PRO_MODELS = new Set(['claude-sonnet-4-5', 'google/gemini-2.5-pro']);
...
app.post('/v1/sessions/start', authenticateToken, async (req, res) => {
  try {
    ...
    if (!MODEL_ALLOWLIST.has(rawModel)) {
      return res.status(400).json({ error: 'Unsupported model', model: rawModel });
    }
    ...
    const selectedVoice = typeof voice === 'string' && voice.trim().length > 0 ? voice.trim() : DEFAULT_VOICE;
    const toolCallingEnabled = typeof tool_calling_enabled === 'boolean' ? tool_calling_enabled : true;
    const webSearchEnabled = typeof web_search_enabled === 'boolean' ? web_search_enabled : true;
    const preferredLanguage = typeof language === 'string' && language.trim().length > 0 ? language.trim() : undefined;
    ...
    console.log(`  Tool calling: ${toolCallingEnabled ? 'enabled' : 'disabled'} | Web search: ${webSearchEnabled ? 'enabled' : 'disabled'}`);
